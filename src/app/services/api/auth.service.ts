...
export class AuthService {
  ...
  set setToken(token: IAuth) {
    const endTime =
      this.milisecondToMinute(new Date().getTime()) + token.expires_in;
  ...
  public isLoged(): boolean {
    const time = this.milisecondToMinute(new Date().getTime());
    const authResponse = this.getAuthResponse;
    if (authResponse?.endTime) return time < authResponse?.endTime;
    return false;
  }

  private milisecondToMinute(mil: number): number {
    return mil / 1000 / 60;
  }
}
